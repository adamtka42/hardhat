...
export class Resolver {
  ...
  public async resolveProjectSourceFile(
    ...
    if (!absolutePath.startsWith(this._projectRoot)) {
      throw new HardhatError(ERRORS.RESOLVER.FILE_OUTSIDE_PROJECT, {
        file: pathToResolve,
      });
    }

    if (absolutePath.includes("node_modules")) {
      throw new HardhatError(ERRORS.RESOLVER.LIBRARY_FILE_NOT_LOCAL, {
        file: pathToResolve,
      });
    }
  ...
  public async resolveLibrarySourceFile(
    ...
    if (!absolutePath.startsWith(libraryPath)) {
      ...
      if (absolutePath.includes(path.join("node_modules", libraryName))) {
        throw new HardhatError(ERRORS.RESOLVER.LIBRARY_FILE_NOT_FOUND, {
          file: globalName,
        });
      }

      throw new HardhatError(ERRORS.RESOLVER.FILE_OUTSIDE_LIB, {
        file: globalName,
        library: libraryName,
      });
    }
